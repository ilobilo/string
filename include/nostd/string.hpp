...
// Just in case, because we can't ignore some warnings from `-Wpedantic` (about
// zero size arrays and anonymous structs when gnu extensions are disabled) on
// gcc
...
namespace nostd
{
    namespace detail
    {
        ...
        struct is_allocator<
            Allocator, std::void_t<typename Allocator::value_type,
    ...
    class basic_string
    {
      ...
      private:
        ...
        constexpr void short_init()
        {
            ...
            {
                this->_allocator.deallocate(buffer,
                                            this->storage._long.cap + 1);
                buffer = nullptr;
            }
        ...
        {
            if (fits_in_sso(size)) this->short_init();
            else this->long_init();
        }
        ...
        {
            if (this->is_long()) this->storage._long.size = size;
            else this->storage._short.size = size & 0x7F;
        }

        constexpr size_type get_cap() const noexcept
        {
            if (this->is_long()) return this->storage._long.cap;
            else return min_cap;
        }
        ...
        {
            if (new_cap == this->storage._long.cap) return;
                                        ...
                                        size_type oldcount, size_type count)
        {
            if (this->addr_in_range(str))
            {
                ...
                this->internal_replace_impl(
                    [&]() {
                        Traits::copy(this->get_data() + pos, rstr.data(),
                                     count);
                    },
                    pos, oldcount, count);
            }
                ...
                this->internal_replace_impl(
                    [&]() { Traits::copy(this->get_data() + pos, str, count); },
                    pos, oldcount, count);
        }
        ...
        {
            this->internal_replace_impl(
                [&]() { Traits::assign(this->get_data() + pos, count, ch); },
                pos, oldcount, count);
        }
                                            ...
                                            bool copy_old)
        {
            if (fits_in_sso(size))
            {
                if (this->is_long() == true) this->short_init();
            ...
            {
                if (this->is_long() == false) this->long_init();
                                       ...
                                       bool copy_old = false)
        {
            if (this->addr_in_range(str))
            {
                ...
                this->internal_assign_impl(
                    [&](auto data) { Traits::copy(data, rstr.data(), size); },
                    size, copy_old);
            }
            else
                this->internal_assign_impl([&](auto data)
      ...
      public:
            ...
            : _allocator(a)
        {
            _NOSTD_STRING_ASSERT(
                count <= this->max_size(),
                "nostd::basic_string::basic_string(): constructed string size "
        ...
        constexpr basic_string(const Type& t, size_type pos, size_type count,
                               const allocator_type& a = allocator_type())
            : _allocator(a)
        {
        ...
        explicit constexpr basic_string(const Type& t, const allocator_type& a
                                                       = allocator_type())
            : _allocator(a)
        {
            sview_type sv(t);
        ...
        // constexpr basic_string(basic_string &&str, size_type pos, size_type
        // count, const allocator_type &a = allocator_type()); constexpr
        // basic_string(basic_string &&str, size_type pos, const allocator_type
        // &a = allocator_type());
        ...
        constexpr basic_string(std::from_range_t, Range&& range,
            ...
            : basic_string(std::ranges::begin(range), std::ranges::end(range),
                           a)
        {
        }
#endif
        ...
        {
            _NOSTD_STRING_ASSERT(count <= this->max_size(),
                                 "nostd::basic_string::basic_string(): "
                                 "resulted string size would exceed max_size()",
                                 std::length_error);
            this->internal_assign(ch, count);
            return *this;
        }
        ...
        constexpr basic_string& assign(basic_string&& str) noexcept(
            ...
            || alloc_traits::is_always_equal::value)
        {
            if (this->_allocator == str._allocator) this->swap(str);
            else this->internal_assign(str.data(), str.size());
        ...
        {
            _NOSTD_STRING_ASSERT(count <= this->max_size(),
                                 "nostd::basic_string::assign(): resulted "
                                 "string size would exceed max_size()",
                                 std::length_error);
            this->internal_assign(str, count);
            return *this;
        }
        ...
        {
            auto len = std::distance(first, last);
            _NOSTD_STRING_ASSERT(len <= this->max_size(),
                                 "nostd::basic_string::assign(): resulted "
                                 "string size would exceed max_size()",
                                 std::length_error);
            this->internal_assign(const_pointer(first), len);
        ...
        template <typename Type>
            requires(std::is_convertible_v<const Type&, sview_type>
                     && !std::is_convertible_v<const Type&, const Char*>)
        constexpr basic_string& assign(const Type& t, size_type pos,
                                       size_type count = npos)
        {
        ...
        {
            auto str = basic_string(std::from_range, std::forward<Range>(range),
                                    this->_allocator);
            _NOSTD_STRING_ASSERT(str.get_size() <= this->max_size(),
                                 "nostd::basic_string::assign_range(): "
                                 "resulted string size would exceed max_size()",
                                 std::length_error);
            return this->assign();
        }
        ...
        {
            _NOSTD_STRING_ASSERT(pos < this->get_size(),
        ...
        constexpr reference       front() { return this->get_data()[0]; }

        constexpr const_reference front() const { return this->get_data()[0]; }
        ...
        constexpr value_type*       data() noexcept { return this->get_data(); }
        ...
        constexpr iterator       begin() noexcept { return this->get_data(); }
        ...
        constexpr bool empty() const noexcept { return this->get_size() == 0; }

        constexpr size_type size() const noexcept { return this->get_size(); }

        constexpr size_type length() const noexcept { return this->get_size(); }
        ...
        {
            _NOSTD_STRING_ASSERT(cap <= this->max_size(),
                                 "nostd::basic_string::reserve(): allocated "
                                 "memory size would exceed max_size()",
                                 std::length_error);
            if (cap <= this->get_cap()) return;

            auto new_cap = std::max(cap, this->get_size());
            if (new_cap == this->get_cap()) return;
        ...
        {
            if (this->is_long() == false) return;

            this->reallocate(this->get_size(), true);
        }

        constexpr void          clear() noexcept { this->set_size(0); }

        constexpr basic_string& insert(size_type pos, size_type count,
                                       value_type ch)
        {
            _NOSTD_STRING_ASSERT(this->get_size() + count <= this->max_size(),
                                 "nostd::basic_string::insert(): resulted "
                                 "string size would exceed max_size()",
                                 std::length_error);
            _NOSTD_STRING_ASSERT(
                pos <= this->get_size(),
        ...
        constexpr basic_string& insert(size_type pos, const value_type* str)
        {
            ...
            _NOSTD_STRING_ASSERT(this->get_size() + len <= this->max_size(),
                                 "nostd::basic_string::insert(): resulted "
                                 "string size would exceed max_size()",
                                 std::length_error);
        ...
        {
            _NOSTD_STRING_ASSERT(this->get_size() + str.get_size()
                                     <= this->max_size(),
                                 "nostd::basic_string::insert(): resulted "
                                 "string size would exceed max_size()",
                                 std::length_error);
        ...
        {
            _NOSTD_STRING_ASSERT(this->get_size() + ilist.size()
                                     <= this->max_size(),
                                 "nostd::basic_string::insert(): resulted "
                                 "string size would exceed max_size()",
                                 std::length_error);
        ...
        constexpr basic_string& insert(size_type pos, const Type& t)
        {
            _NOSTD_STRING_ASSERT(
                ...
                std::out_of_range);
            sview_type sv(t);
            _NOSTD_STRING_ASSERT(this->get_size() + sv.length()
                                     <= this->max_size(),
                                 "nostd::basic_string::insert(): resulted "
                                 "string size would exceed max_size()",
                                 std::length_error);
                                       ...
                                       size_type count = npos)
        {
            ...
            _NOSTD_STRING_ASSERT(this->get_size() + ssv.length()
                                     <= this->max_size(),
                                 "nostd::basic_string::insert(): resulted "
                                 "string size would exceed max_size()",
                                 std::length_error);
        ...
        constexpr iterator insert_range(const_iterator pos, Range&& range)
        {
            ...
            _NOSTD_STRING_ASSERT(this->get_size() + str.get_size()
                                     <= this->max_size(),
                                 "nostd::basic_string::insert_range(): "
                                 "resulted string size would exceed max_size()",
                                 std::length_error);
        ...
        constexpr basic_string& erase(size_type pos = 0, size_type count = npos)
        {
            ...
            _NOSTD_STRING_ASSERT(
                pos <= sz, "nostd::basic_string::erase(): pos out of range",
                std::out_of_range);
        ...
        {
            _NOSTD_STRING_ASSERT(this->get_size() + 1 <= this->max_size(),
                                 "nostd::basic_string::push_back(): resulted "
                                 "string size would exceed max_size()",
                                 std::length_error);
            this->append(1, ch);
        }

        constexpr void          pop_back() { this->erase(this->end() - 1); }

        constexpr basic_string& append(size_type count, value_type ch)
        {
            _NOSTD_STRING_ASSERT(this->get_size() + count <= this->max_size(),
                                 "nostd::basic_string::append(): resulted "
                                 "string size would exceed max_size()",
                                 std::length_error);
        ...
        template <typename Type>
            requires(std::is_convertible_v<const Type&, sview_type>
                     && !std::is_convertible_v<const Type&, const Char*>)
        constexpr basic_string& append(const Type& t, size_type pos,
                                       size_type count = npos)
        {
            sview_type sv(t);
            _NOSTD_STRING_ASSERT(
        ...
        {
            return this->get_view().compare(pos1, count1, str.get_view(), pos2,
                                            count2);
        }
        ...
        template <typename Type>
            requires(std::is_convertible_v<const Type&, sview_type>
                     && !std::is_convertible_v<const Type&, const Char*>)
        constexpr int compare(const Type& t) const noexcept(
        ...
        template <typename Type>
            requires(std::is_convertible_v<const Type&, sview_type>
                     && !std::is_convertible_v<const Type&, const Char*>)
        constexpr int compare(size_type pos1, size_type count1, const Type& t,
                              size_type pos2, size_type count2 = npos) const
        {
            return this->get_view().compare(pos1, count1, sview_type(t), pos2,
                                            count2);
        }
                                        ...
                                        const value_type* str, size_type count2)
        {
            ...
            _NOSTD_STRING_ASSERT(this->get_size() - count + count2
                                     <= this->max_size(),
                                 "nostd::basic_string::replace(): resulted "
                                 "string size would exceed max_size()",
                                 std::length_error);
        ...
        template <typename Type>
            requires(std::is_convertible_v<const Type&, sview_type>
                     && !std::is_convertible_v<const Type&, const Char*>)
        constexpr basic_string& replace(const_iterator first,
                                        const_iterator last, const Type& t)
        {
            sview_type sv(t);
        ...
        {
            _NOSTD_STRING_ASSERT(this->get_size() + count <= this->max_size(),
                                 "nostd::basic_string::resize(): resulted "
                                 "string size would exceed max_size()",
                                 std::length_error);
            auto cap = this->get_cap();
            auto sz  = this->get_size();
        ...
        {
            static_assert(detail::dependent_false<Char>,
                          "nostd::basic_string::resize_and_overwrite(count, "
                          "op) not implemented!");
        }
        ...
        template <typename Type>
            requires(std::is_convertible_v<const Type&, sview_type>
                     && !std::is_convertible_v<const Type&, const Char*>)
        constexpr size_type find_first_of(const Type& t,
                                          size_type   pos = 0) const
            noexcept(std::is_nothrow_convertible_v<const Type&, sview_type>)
        {
        ...
        template <typename Type>
            requires(std::is_convertible_v<const Type&, sview_type>
                     && !std::is_convertible_v<const Type&, const Char*>)
        constexpr size_type find_first_not_of(const Type& t,
                                              size_type   pos = 0) const
            noexcept(std::is_nothrow_convertible_v<const Type&, sview_type>)
        {
        ...
        template <typename Type>
            requires(std::is_convertible_v<const Type&, sview_type>
                     && !std::is_convertible_v<const Type&, const Char*>)
        constexpr size_type find_last_of(const Type& t,
        ...
        template <typename Type>
            requires(std::is_convertible_v<const Type&, sview_type>
                     && !std::is_convertible_v<const Type&, const Char*>)
        constexpr size_type find_last_not_of(const Type& t,
                                             size_type   pos = npos) const
            noexcept(std::is_nothrow_convertible_v<const Type&, sview_type>)
        {
...
#if NOSTD_STRING_NUMERIC_CONVERSIONS
    // numeric conversions
    inline int stoi(const string& str, std::size_t* pos = nullptr,
                    int base = 10)
    {
        auto  cstr = str.c_str();
        ...
        if (pos != nullptr) *pos = cstr - ptr;

        return ret;
    }

    inline long stol(const string& str, std::size_t* pos = nullptr,
                     int base = 10)
    {
        auto  cstr = str.c_str();
        char* ptr  = const_cast<char*>(cstr);
    ...
    inline namespace literals
    {
        inline namespace string_literals
        {
            ...
            constexpr inline string operator""s(const char* str,
                                                std::size_t len)
            {
            ...
            constexpr inline wstring operator""s(const wchar_t* str,
                                                 std::size_t    len)
            {
...
namespace std
{
    ...
    template <typename Allocator>
    struct hash<
        nostd::basic_string<char8_t, std::char_traits<char8_t>, Allocator>>
    ...
    template <typename Allocator>
    struct hash<
        nostd::basic_string<char16_t, std::char_traits<char16_t>, Allocator>>
    ...
    template <typename Allocator>
    struct hash<
        nostd::basic_string<char32_t, std::char_traits<char32_t>, Allocator>>
    ...
    template <typename Allocator>
    struct hash<
        nostd::basic_string<wchar_t, std::char_traits<wchar_t>, Allocator>>
